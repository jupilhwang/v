...
fn benchmark_frame_operations() {
	// Test DATA frame encoding with buffer reuse
	...
	for i in 0 .. iterations {
		...
		start := time.now()
		encoded := v2.encode_frame_to_buffer(frame, mut buf)
...
fn benchmark_large_payloads() {
	...
	mut buf := []u8{len: large_payload_size + 9}
...
fn benchmark_multiple_streams() {
	...
	for stream_id in 1 .. num_streams + 1 {
		for req_id in 0 .. requests_per_stream {
			...
			v2.encode_frame_to_buffer(frame, mut buf)
			total_frames++
		}
	}
