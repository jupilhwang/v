...
fn test_concurrent_requests() {
	...
	// Test multiple concurrent requests using a channel to avoid data races
	n := 5
	ch := chan string{cap: n}

	mut threads := []thread{}
	for _ in 0 .. n {
		threads << spawn make_request(ch)
	}

	// Wait for all threads then close the channel
	for t in threads {
		t.wait()
	}
	ch.close()

	// Drain the channel and count successes
	mut success_count := 0
	for {
		item := <-ch or { break }
		if item.contains('Hello HTTP/2') {
			success_count++
		}
	}

	if success_count > 0 {
		println('  ✓ Concurrent requests test passed (${success_count}/${n} succeeded)')
	} else {
		println('  ⚠ Skipped (no successful requests)')
	}
}

// make_request executes a single HTTP/2 GET and sends the response body to ch.
fn make_request(ch chan string) {
